...
def predict_headings_for_pdf(pdf_path, clf1=None, clf2=None):
    ...
    if not is_training_file:
        ...
        needs_permissive = uniform_font and len(headings_idx) < 5  # Trigger for France-style PDFs with few headings
        ...
        if needs_permissive:
            ...
            for i, block in enumerate(blocks):
                ...
                # Paragraph/fragment exclusion logic (moderate)
                ...
                # H1: Main headings - moderately permissive (reverted to detect "Marseille: The Oldest City in France")
                if (
                    dist_from_top < 0.5 and
                    word_count >= 2 and word_count <= 8 and
                    page <= 3 and
                    len(text) > 4 and
                    not text.endswith(':') and
                    (is_bold or alignment == 1 or line_gap_before > 1)
                ):
                ...
                # H2: Section headings - moderately permissive
                elif (
                      dist_from_top < 0.6 and
                      word_count >= 2 and word_count <= 6 and
                      len(text) > 3 and
                      not text.endswith(':') and
                      (is_bold or alignment == 1 or line_gap_before > 1)
                ):
